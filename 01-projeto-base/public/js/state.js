...
export function getVisiblePatients() {
  ...
  let filtrados = state.patients.filter((patient) => {
    const matchesTerm = patient.name.toLowerCase().includes(term);
    const matchesStatus = state.onlyActive ? patient.active : true;
    return matchesTerm && matchesStatus;
  });

  let mapeados = filtrados.map((patient) => {
    let age = new Date().getFullYear() - new Date(patient.birthDate).getFullYear();
    
    const mesAniv = new Date(patient.birthDate).getMonth();
    const mesAtual = new Date().getMonth();
    
    if (mesAtual < mesAniv || (mesAtual === mesAniv && new Date().getDate() < new Date(patient.birthDate).getDate())) {
      age--;
    }
  ...
  return mapeados.sort((a,b) => a.name.localeCompare(b.name));
}
