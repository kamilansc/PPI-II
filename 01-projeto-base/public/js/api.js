...
export async function getPatientEncounters(id) {
  ...
  if (!response.ok) {
    throw new Error(`Falha ao buscar os atendimentos do paciente (HTTP ${response.status})`);
  }
...
export async function createPatientEncounter(patientId, encounterData) {
  ...
  if (!response.ok) {
    throw new Error(`Falha ao cadastrar o atendimento do paciente (HTTP ${response.status})`);
  }
